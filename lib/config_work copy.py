...
#    if struct_merging_active:
    if source.get('_filename', '') == 'test_struct.yaml':
        logger.warning("merge {}: source={}, destination={}, source_name={}, dest_name={}".format(source.get('_filename', ''), dict(source), dict(destination), source_name, dest_name))
        ext_logging = True
    else:
        ext_logging = False
...
def add_struct_to_template(path, struct_name, template, struct_dict, instance):
    ...
    if struct is None:
        ...
        #logger.debug("- add_struct_to_template: struct_dict = {}".format(dict(struct_dict)))
        logger.debug("- add_struct_to_template: {} <- struct = {}".format(path, struct_name))
        ...
        if instance != '' or True:
            ...
            logger.debug("- add_struct_to_template: subtree={}, instance={}".format(subtree, instance))
